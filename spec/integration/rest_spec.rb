...
describe 'Integration:' do
  ...
  describe 'applications' do
    it 'can be created with the REST API' do
      response = rest_api_post('/applications/')
    ...
    it 'can be listed with the REST API' do
      # Create two apps
      rest_api_post('/applications/')
      rest_api_post('/applications/')
      # list them
      response = rest_api_get('/applications/')
    ...
    it 'can be deleted with the REST API' do
      ...
      # delete one
      delete_response = rest_api_delete('/applications/' + apps_before_delete[0]['app_id'].to_s)
      # list them again
      response = rest_api_get('/applications/')
    ...
    it 'can change their token on request via the API' do
      ...
      # change its token
      change_token_response = rest_api_post('/applications/' + app_before_change['app_id'].to_s + '/generate_new_token')
